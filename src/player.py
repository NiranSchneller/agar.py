...
class Player:
    def __init__(self, name):  # velocity measured in pixels per second
        ...
        self.x = random.randint(
            self.radius, PlatformConstants.PLATFORM_WIDTH - self.radius)
        self.y = random.randint(
            self.radius, PlatformConstants.PLATFORM_HEIGHT - self.radius)
    """
    ...
    def draw(self, color, surface, coordinate_helper):
        screen_radius = coordinate_helper.platform_to_screen_radius(
            self.radius)
    ...
    def eat(self):
        ...
        self.radius = ((math.pi * self.radius ** 2 + math.pi *
                       EdibleConstants.EDIBLE_RADIUS ** 2) / math.pi) ** 0.5
